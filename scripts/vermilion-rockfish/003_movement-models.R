...
files_list <- list.files("data/crawl", pattern='VR')
...
for( file in files_list) { 
  ...
  for( it in c(1,3)) {
    ...
    write.csv(hmm.2, paste0("data/behaviors/VR_states-",it,"mpermin-",time,"min.csv"), row.names=FALSE)
  } 
}

# ok so now lets plot some distributions here. 

list_completed <- list.files("data/behaviors/", pattern='VR')
...
for( file in list_completed){ 
  ...
  barplot(plt$prop, ylim=c(0,1), ylab='', names.arg=plt$labels, col="darkorange2")
