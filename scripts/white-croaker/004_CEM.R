...
ddt<-readOGR("data/shapefiles/total_ddt_pv.shp")
tox.list<-list(ddt)
names(tox.list)<-c("ddt")

list<-list.files("data/behaviors/", pattern="WC")
...
list<-list.files('data/CEMs', pattern='raw_WC')
