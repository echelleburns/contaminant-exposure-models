...
files_list <- list.files("data/crawl", pattern='WC')
...
for( file in files_list) { 
  ...
  proj4string(df)<-CRS("+proj=aeqd +lat_0=33.7141883333 +lon_0=-118.339436667 +x_0=5000 +y_0=5000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")
  ...
  for( it in c(1,3)) {
    ...
    for( h in unique(newalldat$DETECTEDID) ) { # for each listed file...
      ...
      # Find what state it's in
       ...
       for (i in 2:nrow(prepped)-1) { # for each row (except the first and the last)
         if(prepped$stepinm[i] <= step_guard) { # if the step length is less than or equal to 15 m
           prepped$state[i]<-"not-moving" # it's not moving
         } # end for if the step length is less than or equal to 15 m
         if (prepped$stepinm[i] > step_guard ) {  # if the step length is greater than 15m
         ...
         } # end for if step length is greater than 15m
    ...
    write.csv(hmm.2, paste0("data/behaviors/WC_states-",it,"mpermin-",time,"min.csv"), row.names=FALSE)
  } 
}

# ok so now lets plot some distributions here. 

list_completed <- list.files("data/behaviors/", pattern='WC')
...
for( file in list_completed){ 
  ...
  barplot(plt$prop, ylim=c(0,1), ylab='', names.arg=plt$labels, col="thistle3")
