...
#all.dat<-read.csv("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/vps-data/states_5m-buffer-5min.csv")
ddt<-readOGR("D:/Documents/Lowe/OCSD/Pubs/SETAC/OCSD-ddt_clipped.shp")
pcb<-readOGR("D:/Documents/Lowe/OCSD/Pubs/SETAC/OCSD-pcb_clipped.shp")
tox.list<-list(pcb, ddt)
names(tox.list)<-c("pcb","ddt")

list<-list.files("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/vps-data/", pattern="min.csv")

for (g in list) { 
  all.dat<-read.csv(paste0("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/vps-data/",g))
...
name<-colsplit(g, "[.]", c("",""))[,1]
write.csv(tox.dat, paste0("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/vps-data/", name, "_CEM_", names(tox.list)[h], ".csv"), row.names=F)
    # save the csv in the CEM folder
}
} 
  
list<-list.files('D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/vps-data/', pattern='CEM')

for( h in list) { 

dat<-read.csv(paste0("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/vps-data/", h))
...
for( i in unique(dat$ID)) { 
  ...
  timediff<-as.numeric(max(temp$date, na.rm=T)-min(temp$date, na.rm=T))/360
  ...
  totals.temp<-cbind(paste(temp$ID[1]), paste(sum1), paste(scaled_year), paste(scaled_moveyear))
  totals<-rbind(totals, totals.temp)
}

totals<-as.data.frame(totals)
colnames(totals)<-c("ID", "total_tox", "tox_peryear_noscaler", "tox_peryear_scaled")
...
# png(paste0("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/figures/", Sys.Date(), new_name, '_0removed.png'), height=480*12, width=480*10,  res=72*6)
# par(mfrow=c(2,1), cex=1.5, mar=c(2,3,2,5), cex.axis=2)
# barplot(height=plotthis$contam/sum(plotthis$contam), names.arg=plotthis$x, ylim=c(0,1), col="#08519c")
# barplot(height=plotthis2$contam/sum(plotthis2$contam), names.arg=plotthis2$x, ylim=c(0,1), col="#08519c")
# dev.off()
...
# png(paste0("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/figures/", Sys.Date(), "_", this_contam, "_collected_0removed.png"), height=480*6, width=480*10,  res=72*6)
# par(cex=1.5, mar=c(2,3,2,5), cex.axis=2)
# barplot(height=plotthis3$contam/sum(plotthis3$contam), names.arg=plotthis3$x, ylim=c(0,1), col="gray")
# dev.off()

# # remove 0
# totals<-totals[which(totals$round_no > 0 & totals$round_sc > 0),]
# collect<-collect[which(collect$round > 0),]

# Stats tests to the max
sink(paste0("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/", Sys.Date(), "_", new_name, "_results.txt"))
print('Unscaled v Scaled')
print(ks.test(totals$round_no, totals$round_sc))
print('---------------')
print('Scaled vs Tissue')
print(ks.test(totals$round_sc, collect$round))
print('---------------')
print('Unscaled vs Tissue')
print(ks.test(totals$round_no, collect$round))
sink()
