...
vps_data <- read.csv("D:/Documents/Lowe/OCSD/Pubs/SETAC/Hornyhead/vps-data/all_hornyhead.csv")
...
ggplot(all_15, aes(raw_counts, vps_counts, colour = Transmitter)) +
  ...
  ylab("Count of VPS Positions") + ggtitle("15 Minute Bins - HT")

nrow(all_15[which(all_15$raw_counts > 0 & all_15$vps_counts == 0),])/nrow(all_15)
  # 0.866 for rockies; 0.475 for hornies
...
ggplot(all, aes(raw_counts, vps_counts, colour = Transmitter)) +
  geom_jitter() + theme(legend.position="none") + xlab("Count of Raw Detections") +
  ylab("Count of VPS Positions") + ggtitle("15 Minute Bins - HT") + ylim(0, max(all$vps_counts))
