# grab hornyhead raw VPS data after filtering. 
library(reshape)

options(digits=20)

fish<-read.csv("D:/Documents/Lowe/OCSD/Data/Tagging/Burns_Echelle_fishtagged.csv")
fish<-fish[which(fish$Species == 'P. verticalis' & fish$mortality == ''),]
...
write.csv(dat, row.names = F, "data/all_detections/all_HT.csv")
